...
# d is always the plain, unflipped difference (A's value minus B's value) —
# no sign trick shown in the arithmetic. For a minimized criterion, "sign"
# instead mirrors the preference-function curve itself when it's plotted
# below, so the chart directly shows preference rising toward smaller (more
# negative) differences, rather than hiding that behind a "-(...)" in the formula.
raw_d = v_a - v_b
p_value = compute_preference(sign * raw_d, criterion.preference_function, q=criterion.q, p=criterion.p, s=criterion.s)

st.divider()
st.subheader(t("step.step1_subheader"))
if criterion.direction == Direction.MAX:
    direction_word = f":green[**{t('step.maximized')}**]"
else:
...
st.latex(rf"d = {v_a:g} - {v_b:g} = {raw_d:g}")
st.latex(rf"P_{{{crit_name}}}(\text{{{alt_a}}}, \text{{{alt_b}}}) = {p_value:.3f}")

col_vals = values[crit_name].to_numpy()
raw_diffs = [col_vals[a] - col_vals[b] for a in range(len(col_vals)) for b in range(len(col_vals)) if a != b]
span = max([abs(x) for x in raw_diffs] + [criterion.q, criterion.p, criterion.s, 1.0])
# The curve is plotted in raw-difference space. For a maximized criterion that's
# already the preference function's own axis, so the interesting (rising) part
# sits to the right of 0. For a minimized one, "sign" mirrors the whole curve
# left-right here, so it instead rises to the left of 0 — toward smaller values.
if sign > 0:
    d_grid = np.linspace(-0.25 * span, 1.15 * span, 300)
else:
    d_grid = np.linspace(-1.15 * span, 0.25 * span, 300)
p_grid = [
    compute_preference(sign * float(x), criterion.preference_function, q=criterion.q, p=criterion.p, s=criterion.s)
    for x in d_grid
]
...
fig.add_trace(
    go.Scatter(
        x=[raw_d], y=[p_value], mode="markers+text", marker=dict(size=12, color="#e34948", line=dict(width=2, color="white")),
        text=[f"{alt_a} vs {alt_b}"], textposition="top center", name="Your pair",
    )
)
for label, thresh in (("q", criterion.q), ("p", criterion.p)):
    if label in needed:
        fig.add_vline(x=(thresh if sign > 0 else -thresh), line_dash="dot", line_color="gray", annotation_text=label)
fig.update_layout(
    xaxis_title=f"d ({alt_a} − {alt_b})", yaxis_title=f"P_{crit_name}(A, B)",
